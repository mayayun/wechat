<template>
  <div class='hello'>
    ...
    <div class='container'>
      ...
      <p class='margin'></p>
      <p class='addAddIcon' v-if="addr.phone == undefined"><img  src='../images/location.png' alt=''>添加收货地址<router-link to='/addr'><img src='../images/to.png' alt=''></router-link></p>
      <p class='addAddIcon' v-else>
        ...
        <router-link to='/addr'><img src='../images/to.png' alt=''></router-link>
      </p>
      <p class='margin'></p>
        ...
        <ul>
          ...
          <li>
            <dl v-for="(p,index) in p_list" style="padding:0.2rem 0;border-bottom:1px solid #eee">
              <dd><img style='width: 100%;height: auto;' src='../images/logistics.jpg' alt=''></dd>
              <dt>
                <p>天天特价，好吃的坚果糕，自己吃，送大礼都可以，礼盒装，好吃又实惠</p>
                <p style='overflow: hidden;margin-top: 0.1rem'><span class='red left' >￥100</span><span class='right' style="width:auto">X1</span></p>
              </dt>
            </dl>
          </li>
        </ul>
      ...
      <p class='margin'></p>
      <ul class='totel'>
        <li>商品金额<span>100元</span></li>
        <li>运费<span>包邮</span></li>
      </ul>
      <footer class='well'><p></p><p><span class='red' style='font-size: 0.25rem;display: inline-block;margin-right: 1rem;'>合计：100元</span><a href='javascript:void(0)' v-on:click='isPay = !isPay'>去支付</a></p></footer>
    </div>
  ...
  methods : {
    add_order() {
      ...
      var param = {
        "customer": "3",
        "branch": "1",
        "customerAddress": sessionStorage.getItem("add"),
        "leaveMessage": this.leaveMessage,
        "commoditys": [
          {"commodityFormat":3,"count":1}
        ],
        "trackingNumber": ""
      }
      this.$http.post("http://114.215.220.241/WeChat/orders/",param,{headers:{"token":"spTzwiB6JryhOQsB5J1pilh1Vl3iLnEP"}}).then(req => {
        this.$router.replace(`/order_detail/${req.data.id}`)
      })
    }
  },
  mounted() {
    var self = this;
    if(sessionStorage.getItem("add")!=undefined){
      this.$http.get(`http://114.215.220.241/WeChat/customerAddresses/${sessionStorage.getItem("add")}/`,{headers:{"token":sessionStorage.getItem("token")}}).then(req=>{
        self.addr = req.data
      })
    }
